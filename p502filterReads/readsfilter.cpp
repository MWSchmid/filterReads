...
//! THIS VERSION USES THE ENTIRE READ - NO MMs ALLOWED
/*
...
//    }
    ...
    std::cout << "mapping stats - removed in total " << totalRemoved << " reads." << std::endl << std::flush;
    ...
    while (countIter != matchCounter.constEnd()) {
        ...
        std::cout << seqName.toStdString() << "\t" << seqCount << std::endl << std::flush;
        ++countIter;
    }
...
void readsFilter::run() {
    ...
    //! we define a (sub-)read size
    int subReadSize = 30;

    //! we still need to get the real read size to determine once for all reads the starting points
    int readSize = 0;
    while (true) {
        // start control the first buffer (INPUT)
        this->_mutexINPUT->lock();
    ...
    readSize = read.BASES.length();
    std::cerr << "detected a read size of " << readSize << " bases." << std::endl << std::flush;
    std::cerr << "using a subread size of " << subReadSize << " bases." << std::endl << std::flush;
    ...
    while (iter != references.sequences.constEnd()) {
        seqName = iter.key();
        seq = iter.value();
        for (int i = 0; i < (seq.length()-subReadSize); ++i) {
            subSeq = seq.mid(i, subReadSize);
            seqMap.insert(subSeq, seqName);
        }
        ++iter;
    }
...
//    }
    ...
    //! NOTE that we did not remove the first read before - so it is not necessary to process it here
    QString subRead = "";
    int numContained = readSize/subReadSize; //! THATS INTENTIONALLY AN INTEGER DIVISION
    int basesRemaining = (readSize % subReadSize)-1; // minus one as we need it as index
    std::cerr << "will split the reads into " << numContained << " subreads." << std::endl << std::flush;
    std::cerr << "starting points are 0 and " << basesRemaining << "." << std::endl << std::flush;
    ...
    while ( true ) {
        ...
        //! go through the individual subreads starting from zero
        for (int i = 0; i <= numContained; ++i) {
            subRead = read.BASES.mid(i*subReadSize, subReadSize);
            if (seqMap.contains(subRead)) {
                readMatches = true;
                ++totalRemoved;
                mapIter = seqMap.constFind(subRead);
                while (mapIter != seqMap.constEnd() && mapIter.key() == subRead) {
                    seqName = mapIter.value();
                    ++matchCounter[seqName];
                    ++mapIter;
                }
        ...
        //! go through the individual subreads starting from the end
        if (!readMatches) {
            for (int i = 0; i <= numContained; ++i) {
                subRead = read.BASES.mid((i*subReadSize)+basesRemaining, subReadSize);
                if (seqMap.contains(subRead)) {
                    readMatches = true;
                    ++totalRemoved;
                    mapIter = seqMap.constFind(subRead);
    ...
    std::cout << "mapping stats - removed in total " << totalRemoved << " reads (note that the total is accurate, the individual numbers not necessarily (break))." << std::endl << std::flush;
    // write out the match stats
    QMap<QString, int >::const_iterator countIter = matchCounter.constBegin();
    int seqCount = 0;
    while (countIter != matchCounter.constEnd()) {
